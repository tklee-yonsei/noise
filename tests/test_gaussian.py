import unittest
import numpy as np
from noise.gaussian import GaussianNoiseAdder  # GaussianNoiseAdder 클래스 임포트

class TestGaussianNoiseAdder(unittest.TestCase):
    """GaussianNoiseAdder 클래스의 유닛 테스트를 위한 클래스입니다."""

    def setUp(self):
        """각 테스트 전에 실행되는 설정 메서드입니다."""
        self.adder = GaussianNoiseAdder()
    ...
    def test_add_noise(self):
        ...
        self.assertFalse(np.array_equal(noisy_symbols, symbols), "노이즈가 추가되지 않았습니다.")
    ...
    def test_process_request(self):
        ...
        self.assertFalse(np.array_equal(noisy_symbols, symbols_real_imag), "노이즈가 추가되지 않았습니다.")
